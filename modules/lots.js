// modules/lots.js  — single click handler + edit-in-place (no duplicate empty lots)
export function initLotsUI(store){
  const tbody = document.querySelector('#lotsTable tbody');

  // form fields
  const idField = document.getElementById('lot_id');     // hidden
  ...
  const resetBtn= document.getElementById('resetLotBtn');
  ...
  async function saveLot(){
    ...
    // if editing, keep created_at and anything not changed
    const prev = id ? (store.state.lots || []).find(x => x.id === id) || {} : {};

    // prevent creating a totally empty new lot
    const isEditing = !!id;
    if (!isEditing){
      const meaningful =
        (start.value || check.value || Number(qtyT.value||0) || Number(qtyR.value||0) ||
         (mix.value||'').trim() || (notes.value||'').trim());
      if (!meaningful) { alert('Nothing to save. Please fill at least one field.'); return; }
    }
  ...
  // -------- ONE LISTENER ONLY + submit lock (prevents double-save) --------
  let isSavingLot = false;
  function onSaveClick(e){
    e.preventDefault();
    if (isSavingLot) return;
    isSavingLot = true;
    Promise.resolve(saveLot())
      .catch(err => console.error(err))
      .finally(() => { isSavingLot = false; });
  }
  ...
  if (saveBtn) {
    ...
    document.addEventListener('click', (e) => {
      const btn = e.target.closest('#saveLotBtn,[data-action="save-lot"]');
      if (!btn) return;
      onSaveClick(e);
    });
  }
  ...
  if (qtyT) qtyT.oninput = successBadge;
  if (qtyR) qtyR.oninput = successBadge;
